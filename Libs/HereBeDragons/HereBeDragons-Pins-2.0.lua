...
local MAJOR, MINOR = "HereBeDragons-DQTC-Pins-2.0", 18
assert(LibStub, MAJOR .. " requires LibStub")

local pins, oldversion = LibStub:NewLibrary(MAJOR, MINOR)
if not pins then return end

local HBD = LibStub("HereBeDragons-DQTC-2.0")
...
if not pins.worldmapPinsPool then
    ...
    if CreateUnsecuredRegionPoolInstance then
        pins.worldmapPinsPool = CreateUnsecuredRegionPoolInstance("HereBeDragonsPinsTemplateDQTC")
    else
...
WorldMapFrame.pinPools["HereBeDragonsPinsTemplateDQTC"] = worldmapPinsPool

-- provider base API
function worldmapProvider:RemoveAllData()
    self:GetMap():RemoveAllPinsByTemplate("HereBeDragonsPinsTemplateDQTC")
end

function worldmapProvider:RemovePinByIcon(icon)
    for pin in self:GetMap():EnumeratePinsByTemplate("HereBeDragonsPinsTemplateDQTC") do
...
function worldmapProvider:HandlePin(icon, data)
    ...
    if x and y then
        self:GetMap():AcquirePin("HereBeDragonsPinsTemplateDQTC", icon, x, y, data.frameLevelType)
    end
end
